...
describe('UpdateProfile', () => {
  ...
  it('should be able to update the user profile', async () => {
    ...
    expect(updatedUser.name).toBe('Marcelino');
    expect(updatedUser.email).toBe('marcelino@example.com');
  ...
  it('should NOT be able to update the user password using wrong old password', async () => {
    const user = await fakeUsersRepository.create({
      name: 'John Doe',
      email: 'johndoe@example.com',
      password: '123456',
  ...
  it('should NOT be able to update the user without a valid user id', async () => {
    expect(
      updateProfile.execute({
